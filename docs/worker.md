...
学習モーダル（対象サービス・困っていたこと・方針の入力フィールド）
  ※「困っていたこと」はFAQ質問列と同じ粒度の短い質問文で生成される（例: 「ポータルを非表示にする方法は？」）
  ↓
「登録」クリック
  ├→ Slackスレッドに学習データ投稿
...
| 処理 | 旧方式 | 現方式 |
|------|--------|--------|
| AIへの入力 | ドキュメント全文（数十KB） | 見出し一覧 + FAQ質問一覧（回答列は省略）+ 学習データ |
| AIの出力 | ドキュメント全文 | 追加先セクション名 + 追加内容のみ |
| ドキュメント更新 | AI出力で全文置換 | コード側で指定セクションに挿入 |
| 重複チェック | なし | FAQ質問一覧をAIに送信し、同じ質問・同じ内容の重複を検知 → `NO_CHANGE` を返す |
