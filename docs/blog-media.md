...
記事テーマは以下のフレームワークで優先度を決定する。サービスごとに具体的な記事マッピングは各サービス情報ファイルで管理する。

- **短期回収グループ**: 担当者の痛みが強い × 契約に近い × 競合が薄い
- **ロングテール回収グループ**: 機能検索・比較検討層 × 上位プラン検討者
...
- **必ず「最短の導線」を作る**: まとめ直前にCTAへの最短導線を1文で示す

### 内部リンクによる回遊構造

以下の流れを基本パターンとする。サービスごとの具体的な記事マッピングは各サービス情報ファイルを参照。
...
| 遷移 | 狙い |
|------|------|
| 共感 → 方法 | 「なぜ起きるのか」に共感させた後、「どう解決するか」へ誘導 |
| 方法 → 機能 | 「解決したい」意欲が高まった状態で、具体機能の深掘りへ誘導 |
| 機能 → 手順 | 機能に興味を持ったユーザーを「すぐ試せる」手順へ誘導 |
| 手順 → LP | 導入イメージができた状態でLPに送り、登録へ誘導 |
