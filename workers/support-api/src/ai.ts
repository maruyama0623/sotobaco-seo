...
4. お問い合わせ内容が曖昧・不明確な場合のルール:
   - お客様が何に困っているのか、何をしたいのかが明確に読み取れない場合は、推測で回答せずヒアリング形式の回答案を作成する
   - 考えられるお悩み・ご要望を3〜5つ、番号付きで具体的に提示する
   - お客様が該当する番号をコピーして返信するだけで済むよう、各選択肢は簡潔でわかりやすい文言にする
   - 以下のテンプレートに従う:
...
お問い合わせいただきました内容について、より的確にご案内させていただくため、お手数ですが以下のうち該当するものをお知らせいただけますでしょうか。
...
該当する番号をご返信いただけましたら、すぐにご案内させていただきます。
何卒よろしくお願い申し上げます。
...
4. 返信内容が曖昧・不明確な場合:
   - お客様の意図が明確に読み取れない場合は、考えられるお悩み・ご要望を3〜5つ番号付きで提示し、該当するものを返信していただくよう案内する
   - 各選択肢はお客様がそのままコピーして返信できるよう、簡潔でわかりやすい文言にする
   - 最後の選択肢は「上記以外（具体的にお聞かせください）」とする`;
}
